...
suite("transforms", function () {
    var db: MemDb;
    var items: MemDbCollection<{ name: string; owner: string; maker: string }>;

    setup(function () {
        db = M.createDb("transformTest");
        items = db.addCollection<{ name: string; owner: string; maker: string }>("items");
    ...
    test("basic multi-step transform", function () {
        var results = items.chain().find({ owner: "odin" }).where((obj) => obj.name.indexOf("drau") !== -1).data();
    ...
    test("parameterized where", function () {
        var results = items.chain().where((obj) => obj.name.indexOf("nir") !== -1).data();

        asr.equal(results.length, 3);
    });

    test("dynamic view named transform", function () {
        var testColl = db.addCollection<{ a: string; b: number }>("test");
