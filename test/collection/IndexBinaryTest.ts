...
suite("binary indices", function () {
    var testRecords: { name: string; owner: string; maker: string }[];
    ...
    test("collection.clear affects binary indices correctly", function () {
        ...
        var items = db.addCollection<typeof testRecords[0]>("users", { indices: ["name"] });
    ...
    test("binary index loosly but reliably works across datatypes", function () {
        ...
        var dirtydata = db.addCollection<{ a: number; b?: any }>("dirtydata", { indices: ["b"] });
    ...
    test("adaptiveBinaryIndex high level operability test", function () {
        var db = M.createDb("idxtest");
        var coll = db.addCollection<{ customIdx: number; originalIdx?: number; desc?: string }>("users", {
            //adaptiveBinaryIndices: true,
            indices: ["customIdx"]
        });
        ...
        asr.equal(coll.findOne({ customIdx: 300 })?.customIdx, 300);
        asr.equal(coll.findOne({ customIdx: 234 })?.customIdx, 234);
        asr.equal(coll.findOne({ customIdx: 7 })?.customIdx, 7);
        asr.equal(coll.findOne({ customIdx: 567 })?.customIdx, 567);
        ...
        asr.equal(coll.findOne({ customIdx: 100 })?.customIdx, 100);
        asr.equal(coll.findOne({ customIdx: 200 })?.customIdx, 200);
        asr.isNull(coll.findOne({ customIdx: 300 }));
        asr.equal(coll.findOne({ customIdx: 400 })?.customIdx, 400);
        asr.equal(coll.findOne({ customIdx: 500 })?.customIdx, 500);
        asr.isNull(coll.findOne({ customIdx: 7 }));
        asr.equal(coll.findOne({ customIdx: 123 })?.customIdx, 123);
        asr.isNull(coll.findOne({ customIdx: 234 }));
        asr.equal(coll.findOne({ customIdx: 345 })?.customIdx, 345);
        asr.isNull(coll.findOne({ customIdx: 567 }));
    });

    test("adaptiveBinaryIndex high level random stress test", function () {
        var db = M.createDb("idxtest");
        var coll = db.addCollection<{ customIdx: number; sequence: number; desc: string }>("users", {
            //adaptiveBinaryIndices: true,
            indices: ["customIdx"]
        });
        ...
        for (var idx = 0; idx < 1000; idx++) {
            ...
            asr.isNotNull(result);
            asr.deepEqual(result?.customIdx, currId);
        }
    });
...
function notNull<T>(value: T): Exclude<T, null | undefined> {
    asr.isNotNull(value);
    return <any>value;
}