...
suite("Constraints", function () {
    test("should retrieve records with by()", function () {
        ...
        asr.equal((_a = coll.by("username", "joe")) === null || _a === void 0 ? void 0 : _a.name, "Joe");
        var byUsername = coll.by("username");
        asr.equal((_b = byUsername("jack")) === null || _b === void 0 ? void 0 : _b.name, "Jack");
        var joe = coll.by("username", "joe");
        joe.username = "jack";
        asr.throws(function () { return coll.update(joe); }); // "Duplicate key for property username: " + joe.username
        joe.username = "jim";
        coll.update(joe);
    ...
    test("should not add record with null index", function () {
        var db = M.createDb();
        var coll3 = db.addCollection("nullusers", { unique: ["username"] });
    ...
    test("should not throw an error id multiple nulls are added", function () {
        var db = M.createDb();
        var coll4 = db.addCollection("morenullusers", { unique: ["username"] });
    ...
    test("should not crash on unsafe strings", function () {
        var _a;
        var db = M.createDb();
        var coll = db.addCollection("local_storage", { unique: ["key"] });
        asr.isNull(coll.by("key", "hasOwnProperty"));
        coll.insert({ key: "hasOwnProperty", name: "hey" });
        asr.equal((_a = coll.by("key", "hasOwnProperty")) === null || _a === void 0 ? void 0 : _a.name, "hey");
    });
});
